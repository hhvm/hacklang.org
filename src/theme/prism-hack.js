...
(function (Prism) {
	...
	Prism.languages.hack = {
		...
		'keyword': [
			...
			{
				pattern: /(\)\s*:\s*(?:\?\s*)?)\b(?:array(?!\s*\()|bool|callable|(?:false|null)(?=\s*\|)|float|int|iterable|mixed|never|object|self|static|string|void|Awaitable)\b/i,
				alias: 'return-type',
				greedy: true,
				lookbehind: true
			},
	...
	Prism.languages.insertBefore('hack', 'variable', {
		'string': string,
		'attribute': {
			...
			inside: {
				'attribute-content': {
					pattern: /^(#\[)[\s\S]+(?=\]$)/,
					lookbehind: true,
					// inside can appear subset of hack
					inside: {
						'comment': comment,
						'string': string,
	...
	Prism.hooks.add('before-tokenize', function (env) {
		...
		var hackPattern = /<\?(?:[^"'/#]|\/(?![*/])|("|')(?:\\[\s\S]|(?!\1)[^\\])*\1|(?:\/\/|#(?!\[))(?:[^?\n\r]|\?(?!>))*(?=$|\?>|[\r\n])|#\[|\/\*(?:[^*]|\*(?!\/))*(?:\*\/|$))*?(?:\?>|$)/g;
		Prism.languages['markup-templating'].buildPlaceholders(env, 'hack', hackPattern);
	});

	Prism.hooks.add('after-tokenize', function (env) {
		Prism.languages['markup-templating'].tokenizePlaceholders(env, 'hack');
	});
